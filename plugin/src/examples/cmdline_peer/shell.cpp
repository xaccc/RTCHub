...
//	switch(cmd.c_str())
	{
        ...
        if (cmd=="SENDMSG")
        {
			cout << "sendmsg: " << subcmd << endl;
            if (subcmd=="")
            {
                cout << "Illegal form - argument required: sendmsg <peer>:<message>" << endl;
                return false;
            }
            else
            {
                ...
                {
                    namevalues["peername"] = subcmd.substr(0, pos);
                    namevalues["message"] = subcmd.substr(pos+1);
                    //cout << "sendmsg to peer=" << namevalues["peername"] << endl;
                    return true;
                }
            }
        }
